...
void BatchSnbxExportDialog::updateTitle()
{
    ...
    if (count == 1) {
        ...
        m_descLabel->setText(tr("Share the notebook as a .3EN project that can be imported on another device."));
    } else {
        m_titleLabel->setText(tr("Share %1 3ENotes Projects").arg(count));
        m_descLabel->setText(tr("Share %1 notebooks as .3EN projects that can be imported on another device.").arg(count));
    }
#else
    if (count == 1) {
        m_titleLabel->setText(tr("Export 3ENotes Project"));
        m_descLabel->setText(tr("Export the notebook as a .3EN project that can be shared or transferred."));
    } else {
        m_titleLabel->setText(tr("Export %1 3ENotes Projects").arg(count));
        m_descLabel->setText(tr("Export %1 notebooks as .3EN projects that can be shared or transferred.").arg(count));
    }
#endif
}
...
QString BatchSnbxExportDialog::outputDirectory() const
{
    ...
    QDir dir(cacheDir);
    QStringList snbxFiles = dir.entryList(QStringList() << "*.3EN" << "*.3en" << "*.3enotes" << "*.snbx", QDir::Files);
